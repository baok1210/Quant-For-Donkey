...
# --- Cấu hình trang ---
st.set_page_config(
    page_title="🫏 Quant for Donkey Dashboard",
    page_icon="🫏",
    layout="wide"
)
...
view = st.sidebar.radio("Chọn chế độ:", ["Donkey View", "Quant View"])
...
# --- Donkey View (Simple) ---
if view == "Donkey View":
    st.header("🫏 Donkey View - Quyết định đơn giản")
    st.markdown("### 🎯 MUA hay KHÔNG MUA?")
    
    # Simulate decision from main system
    import random
    decision = "BUY" if random.random() > 0.4 else "HOLD"
    ...
    with col_main[2]:
        ...
        st.metric("Risk Level", "LOW" if expected_return < 0.1 else "MODERATE")
    
    # Simple explanation
    st.write("### 📝 Giải thích đơn giản:")
    if decision == "BUY":
        st.success("✅ Hệ thống phát hiện tín hiệu tăng giá mạnh từ các chỉ báo kỹ thuật và dòng tiền thông minh.")
    else:
        st.warning("⚠️ Thị trường đang có dấu hiệu điều chỉnh, nên chờ thêm tín hiệu xác nhận.")
    
    # Action button
    if decision == "BUY":
        st.button("🛒 Execute BUY Order", type="primary")
    else:
        st.button("👀 Watchlist", type="secondary")
